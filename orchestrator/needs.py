...
Four streams converge here so there's one place (and one count) to watch instead of scattered
badges:
  ...
  • proposals — batches of unit-proposed tickets awaiting Approve(-to-file)/Deny (council, meetings,
                out-of-scope in-dev findings); one card per batch (EU-61)
...
def summary(cfg: Config) -> dict:
    ...
    return {
        ...
        "total": (len(decisions_items) + len(approvals_items)
                  + len(proposal_items) + len(task_items)),
    }
