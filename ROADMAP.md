...
- **Scheduled patrols (find → file, unprompted)** — `general patrol <app>` sends the three recon
  ...
  it files tickets). (EU-432: patrol is no longer cron-scheduled on the server — that host has no
  real product repo to patrol; run it on demand, or schedule it yourself on a host with a real
  product checkout.) Verified by a dedicated harness; dashboard suite 66/66.
