...
# === 5. LABEL MAPS RESOLVE TO THE SOT — board / roster / consult labels == display(key) ========
...
# warroom.py — cockpit board roster AND the consult map both resolve via _OFFICER_KEY
...
for uikey, gname in warroom._GROUP_NAME.items():
    ik = warroom._OFFICER_KEY.get(uikey, uikey)
    check(f"consult name resolves to SOT: {uikey!r} -> {gname!r}", gname == display(ik),
