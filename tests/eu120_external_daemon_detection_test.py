...
def _test_external_daemon_stopping_behavior() -> None:
    """An EXTERNAL daemon is never 'stopping' because of a LOCAL stop_event (EU-356).
    ...
    EU-356 gates ``stopping`` on ``internal_on`` (a live in-process loop), not the wider ``on`` (which
    ...
    with patch("orchestrator.autopilot.daemon_running", return_value=True), \
         patch("orchestrator.autopilot.daemon_is_external", return_value=True):
        s = cockpit_state.get_autopilot_status("automatixy")
        chk("external daemon: stopping=False — a local Event can't stand down another process (EU-356)",
