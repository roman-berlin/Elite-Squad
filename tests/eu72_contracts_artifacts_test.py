...
# token_burn is a mutable dict the loop uses to record per-officer usage totals.
store.token_burn["builder"] = 4200
store.token_burn["reviewer"] = 1800
check("token_burn stores per-officer totals",
      store.token_burn == {"builder": 4200, "reviewer": 1800})
store2 = PerTicketArtifactStore()
check("two stores have independent token_burn dicts", store2.token_burn is not store.token_burn)
