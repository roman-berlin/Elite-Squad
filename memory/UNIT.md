...
- 2026-06-18: Green build/typecheck hides cross-environment config defects — every prod `vercel.json` `/api/*` rewrite pointed at the DEV backend while all gates stayed green. Env-parity is now an evidence-backed BLOCKING pre-flight (graduated from observation-only): all prod `/api/*` targets must resolve to the prod backend before cutover.
- 2026-06-18: Migration *mechanics* ≠ migration *outcome* — QM can certify RLS migrations as idempotent/txn-wrapped/rollback-present while the resulting policies fail open and pass every gate. Tenant-RLS graduates to BLOCKING: require a scratch-DB cross-tenant SELECT returning zero rows before prod.
...
- 2026-06-18: Scout (QA) is the unit's true open seam, not any FE hire — mission ends in "QA on DEV" yet that step has no owner and a ticket already sits `running` with nothing to receive it. Scout is next post to activate once the a11y/coverage gate proves out.
...
- 2026-06-16: Inspector A11y and Test-Coverage auditors are calibrated correctly (caught every instance) — they serve as final verifiers, not primary catch. Recurring Inspector bounces signal a missing build-side gate, not a check-side gap.
